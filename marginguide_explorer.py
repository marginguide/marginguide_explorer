...
def db_con():
    con = sqlite3.connect(r"C:\Program Files (x86)\Margin Guide\marginguide_explorer\data\seldb.db")
    # if getattr(sys, 'frozen', False):
    #     con = sqlite3.connect(r"C:\Program Files (x86)\Margin Guide\marginguide_explorer\data\seldb.db")
    # else:    
    #     con = sqlite3.connect(r"C:\Users\JAEYEON\Documents\GitHub\marginguide_project\marginguide_explorer\data\seldb.db")
    return con

def db_conn():
    conn = sqlite3.connect(r"C:\Program Files (x86)\Margin Guide\data\db.db")
    # if getattr(sys, 'frozen', False):
    #     conn = sqlite3.connect(r"C:\Program Files (x86)\Margin Guide\data\db.db")
    # else:    
    #     conn = sqlite3.connect(r"C:\Users\JAEYEON\Documents\GitHub\marginguide_project\data\db.db")
    return conn
...
def rankings():
    try:
        ...
        if len(keywords) > 0:
            ...
            with SB(headless2=headless, uc=True,log_cdp=False, block_images=True, incognito=True, mobile=True, browser="chrome") as self:
                ...
                for keyword in keywords:
                    url = f"https://m.coupang.com/nm/search?q={keyword}"
                    self.open(url)
                    ...
                    while True:
                        ...
                        time.sleep(random.uniform(1, 2) )
                        scroll = random.randint(100, 500) + scroll
                        self.execute_script(f"window.scrollTo(0, {scroll})")
                        time.sleep(random.uniform(1, 2) )
                        scroll = random.randint(100, 500) + scroll
                        self.execute_script(f"window.scrollTo(0, {scroll})")
                        ...
                        items = soup.select(" li.plp-default__item:not(.search-product__cmg-badge):not(.ad-badge-text):not(.search-product__ad-badge) ")
                        ...
                        for index, item in enumerate(items, start=1): #저장한 items들을 순차적으로 검색함.  
                            ...
                            try:dispname = item.find('strong', class_='title').get_text(strip=True)
                            except:continue
                            try:
                                price = item.find('div', class_='discount-price').find('strong').get_text(strip=True)
                                price = price.replace(",", "")
                                price = int(price)
                            except:continue
                            try:
                                thumb = item.find('span', class_="thumbnail").find('img')['src']
                                n = thumb.find('/image/')
